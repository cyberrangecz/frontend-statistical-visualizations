...
export class AxesCreationService {
   ...
   */
  createXAxesTitle(canvasId: string, x: number, y: number, text: string, fontSize: number, textAnchor?: string): void {
  ...
  getAxisFontSize(element: string): number {
    ...
    const rate = 0.022;
    const elementWidth = Number((d3.select(element).select('mat-card').node() as any).getBoundingClientRect().width);
